...
void load_comment(myteams_t *teams, char **array)
{
    ...
    uuid_parse(array[5], uuid);
    insert_comment(&thread->comment_head, array[4], uuid);
    comment = get_comment(thread->comment_head, uuid, array[4],
        strtol(array[3], NULL, 10));
    comment->timestamp = strtol(array[3], NULL, 10);
}
...
void load_thread(myteams_t *teams, char **array)
{
    ...
    channel = get_channel_by_uuid(team->channel_head, uuid);
    uuid_parse(array[5], uuid);
    insert_thread(&channel->thread_head, array[3], array[4], uuid);
    uuid_parse(array[6], uuid);
    thread = get_thread_by_title(channel->thread_head, array[3]);
    uuid_copy(thread->thread_uuid, uuid);
    thread->timestamp = strtol(array[2], NULL, 10);
}
...
void load_channel(myteams_t *teams, char **array)
{
    ...
    channel = get_channel_by_name(team->channel_head, array[1]);
    uuid_parse(array[2], uuid);
